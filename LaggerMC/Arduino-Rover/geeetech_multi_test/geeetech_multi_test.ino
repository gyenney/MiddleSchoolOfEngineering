 ...
 void SendTextMessage()
{
 ...
 delay(100);
 mySerial.println("AT + CMGS = \"xxxxxxxxxx\"");//send sms message, be careful need to add a country code before the cellphone number
 delay(100);
...
void SubmitHttpRequest()
{
 ...
 delay(1000);
 ShowSerialData();
 mySerial.println("AT+SAPBR=3,1,\"APN\",\"phone\"");//setting the APN, the second need you fill in your local apn server
                                                    // for AT&T, APN = "phone" for a windows phone.
                                                    // source: https://www.att.com/esupport/article.html#!/wireless/KB424489 
 delay(4000);
 ShowSerialData();
 mySerial.println("AT+SAPBR=1,1");//setting the SAPBR, for detail you can refer to the AT command mamual
                                  // Only do this once per session.  future attempts will return an error. but remaining steps 
